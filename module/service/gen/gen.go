...
func (g *DefaultGenerator) genBiz(in *parser.Table) (string, error) {
	...
	findAllCode, err := genFindAll(in)
	if err != nil {
		return "", err
	}
	paginateCode, err := genPaginate(in)
	if err != nil {
		return "", err
	}
	findCode = append(findCode, findOneCode, findAllCode, paginateCode)
...
func (g *DefaultGenerator) executeBiz(table *parser.Table, code *code) (*bytes.Buffer, error) {
	text, err := filex.LoadTemplate(serviceGenTemplateFile, template.ServiceGen)
	if err != nil {
		return nil, err
	}
	t := output.With("service").
		Parse(text).
		GoFmt(true)
