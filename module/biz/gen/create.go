...
func genCreate(table *parser.Table) (string, string, error) {
	var count int
	...
	// interface method
	text, err = filex.LoadTemplate(createTemplateInterfaceFile, template.CreateMethodInterface)
	if err != nil {
		return "", "", err
	}

	interfaceBuffer, err := output.With("createInterface").Parse(text).Execute(map[string]interface{}{
		"upperStartCamelObject": camel,
		"data":                  table,
	})
	if err != nil {
